...
class AnalysisPayload(BaseModel):
    ...
    - group_inputs: p2s 模式下的中古分組欄位列表；所有 feature 共用同一組欄位，
                   例如 ['攝', '組'] 表示對每個 feature 都按 攝+組 聯合分組
    ...
    @field_validator('group_inputs', mode='before')
    @classmethod
    def validate_group_inputs_type(cls, v):
        if v is None:
            return v
        if isinstance(v, str):
