...
from app.tools.progress_utils import (
    JYUT2IPA_HEARTBEAT_SECONDS,
    JYUT2IPA_TIMEOUT_SECONDS,
    ProgressHeartbeat,
...
async def get_progress(task_id: str):
    ...
    task = maybe_timeout_task(
        task_manager,
        task_id,
        timeout_seconds=JYUT2IPA_TIMEOUT_SECONDS,
        failure_status=TaskStatus.FAILED,
        timeout_message="Jyut2IPA task timeout: no heartbeat received within the expected window",
        on_timeout=lambda current_task_id: _touch_jyut2ipa_cleanup(current_task_id, "conversion_timeout"),
    )
    if not task:
        raise HTTPException(status_code=404, detail="任务不存在")
