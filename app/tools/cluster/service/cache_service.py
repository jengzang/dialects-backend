...
from app.service.core.new_pho import delete_cache_sync, get_cache_sync, set_cache_sync
...
def get_cached_cluster_result(job_hash: str) -> Optional[Dict[str, Any]]:
    cached = get_cache_sync(build_cluster_result_cache_key(job_hash))
...
def get_inflight_task_id(job_hash: str) -> Optional[str]:
    cached = get_cache_sync(build_cluster_inflight_key(job_hash))
...
def clear_inflight_task_id(job_hash: str, task_id: Optional[str] = None):
    if task_id:
        ...
        if current and current != task_id:
            return
    delete_cache_sync(build_cluster_inflight_key(job_hash))
